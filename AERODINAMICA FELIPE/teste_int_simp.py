...
for caminho_arquivo, nome_df in caminhos_arquivos:
    ...
    # Calcular as áreas utilizando a regra de Simqpson
    area_intra_simps = regra_simpson(x_filtered[:25],y_filtered[:25])
    area_extra_simps = regra_simpson(x_filtered[25:],y_filtered[25:])
...
'''
print(f'Área intradorso: {area_intra_simps}')
    print(f'Área extradorso: {area_extra_simps}')
    print(y_filtered)
'''
...
def calcular_CN(df_areas_simp, c):
    ...
    df_CN_Simp = pd.DataFrame(columns=df_areas_simp.columns)
    
    # Para cada coluna (ângulo) em df_areas_simp, calcular CN e armazenar
    df_CN_Simp.loc[0] = df_areas_simp.loc["Total"] / c  # Dividir a linha "Total" pela corda c
...
df_CN_Simp = calcular_CN(df_areas_simp, c)
